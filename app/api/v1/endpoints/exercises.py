...
from app.models.exercise import Exercise, LibraryExercise, TargetMuscle
...
def get_library_exercises(
    ...
    query = db.query(LibraryExercise)
...
def get_library_exercise(exercise_id: int, db: Session = Depends(get_db)):
    exercise = db.query(LibraryExercise).filter(LibraryExercise.id == exercise_id).first()
    if not exercise:
